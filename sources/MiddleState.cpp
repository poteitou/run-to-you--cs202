#include <MINE/MiddleState.hpp>

MiddleState::MiddleState(StateStack &stack, Context context)
    : State(stack, context),
      mBackgroundSprite(),
      mGroundSprite(),
...
{
    mCntLives = getCount();
    sf::Texture &backgroundTexture = context.mTextures->get(Textures::OrangeBG);
...
int MiddleState::randomInt(int l, int r)
{
    std::mt19937 rd(std::chrono::steady_clock::now().time_since_epoch().count());
    return l + rd() % (r - l + 1);
}

void MiddleState::createObstacle()
{
    if (((int)mDistance / 100) > 570.f)
        return;
...
bool MiddleState::handleEvent(User user)
{
...
bool MiddleState::update(sf::Time dt)
{
    if (mIsPaused)
    {
        mMusic.play();
...
void MiddleState::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
